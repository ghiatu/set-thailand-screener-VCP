...
Ported from xang1234/stock-screener (Apache-2.0 license):
https://github.com/xang1234/stock-screener/blob/main/backend/app/analysis/patterns/legacy_vcp_detection.py

Zero framework dependencies (pandas + numpy only) — safe to drop straight
into a lightweight, cron/GitHub-Actions-driven screener like RyanJHamby's.
...
IMPORTANT: all price/volume Series passed in must be ordered
most-recent-first (index 0 = today). yfinance returns oldest-first,
so reverse it before calling: `prices.iloc[::-1]`. See the usage
